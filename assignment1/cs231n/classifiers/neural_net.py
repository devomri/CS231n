...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    relu_activation = np.maximum(hidden_layer, 0)
    ...
    softmax_loss = -np.log(softmax[np.arange(N), y])
    data_loss = np.average(softmax_loss)
    ...
    grads['W1'] = np.dot(X.T, dhidden_layer)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      minibatch_indices = np.random.choice(np.arange(num_train), batch_size)
